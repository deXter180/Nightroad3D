...
namespace Broccoli.Catalog
{
	...
	public class SproutCatalog {
		...
		/// <summary>
		/// Validates if this catalog is suitable to use for a descriptor implementation.
		/// </summary>
		/// <param name="descriptorImpl">Descriptor implementation id.</param>
		/// <returns><c>True</c> if the catalog is suitable for the implementation.</returns>
		public virtual bool ValidateDescriptorImp (int descriptorImpl) {
			if (descriptorImpl == BranchDescriptorCollection.IMPL_NONE ||
				descriptorImpl == IMPL_BRANCH)
			{
				return true;
			}
			return false;
		...
		public bool LoadPackages () {
			...
			if (!isPackLoaded) {
				...
				Debug.Log ("isPackageLoaded: " + isPackLoaded + " (" + items.Count + ")");
				PrepareGUIContents ();
				isPackLoaded = true;
				return true;
			}
