...
namespace Broccoli.BroccoEditor
{
	...
	public class SproutLabEditor {
		...
		public float debugRescale = 0.1f;
		public string debugLoadPath = string.Empty;
		private Texture2D debugAtlas = null;
		#endregion

		#region Delegates and Events
		public delegate void BranchDescriptorChange (BranchDescriptor branchDescriptor, BranchDescriptorCollection branchDescriptorCollection);
		public delegate void VariationDescriptorChange (VariationDescriptor variationDescriptor, BranchDescriptorCollection branchDescriptorCollection);
		public delegate void ShowNotification (string notification);
		public BranchDescriptorChange onBeforeEditBranchDescriptor;
		public BranchDescriptorChange onEditBranchDescriptor;
		public VariationDescriptorChange onBeforeEditVariationDescriptor;
		public VariationDescriptorChange onEditVariationDescriptor;
		public ShowNotification onShowNotification;
		public void TriggerOnBeforeEditBranchDescriptor (BranchDescriptor branchDescriptor) { onBeforeEditBranchDescriptor?.Invoke (branchDescriptor, branchDescriptorCollection); }
		public void TriggerOnEditBranchDescriptor (BranchDescriptor branchDescriptor) { onEditBranchDescriptor?.Invoke (branchDescriptor, branchDescriptorCollection); }
		public void TriggerOnBeforeEditVariationDescriptor (VariationDescriptor variationDescriptor) { onBeforeEditVariationDescriptor?.Invoke (variationDescriptor, branchDescriptorCollection); }
		public void TriggerOnEditVariationDescriptor (VariationDescriptor variationDescriptor) { onEditVariationDescriptor?.Invoke (variationDescriptor, branchDescriptorCollection); }
		#endregion
		...
		/// </summary>
		private static GUIContent[] debugViewOptions = new GUIContent[5];
		/// <summary>
		...
		private static string labelGirth = "Girth (Radius)";
		...
		private static string labelAlignmentRandom = "Align Randomness";
		...
		public SproutLabEditor (VisualElement parentRootVisualElement) {
			...
			if (GlobalSettings.sproutLabDebugEnabled) {
		...
		public void LoadBranchDescriptorCollection (BranchDescriptorCollection branchDescriptorCollection, SproutSubfactory sproutSubfactory, string path = "") {
			...
			if (branchDescriptorCollection.descriptorImplId < 0) {
				...
				if (currentImplementation != null) {
					...
					currentImplementation.OnLoadBranchDescriptorCollection (branchDescriptorCollection, sproutSubfactory);
		...
		public void RegeneratePreview (int viewMode = VIEW_COMPOSITE) {
			if (canvasStructureView == CanvasStructureView.Snapshot) {
				sproutSubfactory.ProcessSnapshotPipelineMesh ();
		...
		public void SetThirdLevelView (int panelSectionIndex) {
			if (panelSectionIndex != _currentPanelSection) {
		...
		public void SetViewLevels (int firstLevelView, int secondLevelView, int thirdLevelView) {
			...
			if (currentImplementation != null) {
				currentImplementation.ViewChanged (
		...
		public void DrawStructureViewHeader (Rect windowRect) {
			...
			if (GUILayout.Button (generateNewStructureGUI)) {
				if (currentImplementation != null) propagate = currentImplementation.OnGenerateNewStructure ();
				if (propagate) {
					onBeforeEditBranchDescriptor (selectedBranchDescriptor, branchDescriptorCollection);
					sproutSubfactory.ProcessSnapshotPipelineMesh (true, SproutSubfactory.MaterialMode.Composite, true);
					onEditBranchDescriptor (selectedBranchDescriptor, branchDescriptorCollection);
					ShowPreviewMesh ();
				}
			}
			if (GUILayout.Button (regenerateCurrentGUI)) {
				if (currentImplementation != null) propagate = currentImplementation.OnRegenerateStructure ();
				if (propagate) {
					RegeneratePreview ();
				}
			}
			if (GUILayout.Button (loadFromTemplateGUI)) {
				if (currentImplementation != null) propagate = currentImplementation.OnLoadFromTemplate ();
			...
			if (canvasStructureView == CanvasStructureView.Snapshot) {
					...
					&& selectedSproutMap != null && selectedSproutMap.texture != null)
				{
					...
					if (areaCanvas.HasChanged ()) {
						...
						onBeforeEditBranchDescriptor?.Invoke (selectedBranchDescriptor, branchDescriptorCollection);
						areaCanvas.ApplyChanges (selectedSproutMap);
						onEditBranchDescriptor?.Invoke (selectedBranchDescriptor, branchDescriptorCollection);
						meshPreview.backgroundColor = defaultPreviewBackgroundColor;
					}
				}
				// DRAW STRUCTURE MESH.
		...
		public void DrawTemplateView (Rect windowRect) {
			...
			if (GlobalSettings.debugEnabled) {
				string catalogsInfo = catalogs.Count + " catalogs found.";
				catalogsInfo += "\nBranchDesc Impl: " + branchDescriptorCollection.descriptorImplId;
				for (int i = 0; i < catalogs.Count; i++) {
					catalogsInfo += string.Format ("\n  {0} {1} with {2} elements. (Valid Impl: {3})", 
						i, catalogs[i].GetType (), catalogs[i].totalItems, catalogs[i].ValidateDescriptorImp (branchDescriptorCollection.descriptorImplId));
				}
				EditorGUILayout.HelpBox (catalogsInfo, MessageType.None);
			}
			#endif
			...
			if (catalogs.Count > 0) {
				...
				for (int i = 0; i < catalogs.Count; i++) {
					catalog = catalogs [i];
					if (catalog.ValidateDescriptorImp (branchDescriptorCollection.descriptorImplId)) {
		...
		public void DrawStructureViewControlPanel (Rect windowRect) {
			...
			if (currentStructureSettings.hasCustomPanels) {
				currentImplementation.DrawPanels (windowRect);
			}
		...
		public void DrawStructurePanel (Rect windowRect) {
			...
			float sproutANormalRandomness = selectedBranchDescriptor.sproutANormalRandomness;
			...
			float sproutBNormalRandomness = selectedBranchDescriptor.sproutBNormalRandomness;
			int activeLevels = 2;
			if (GlobalSettings.experimentalAdvancedSproutLab) {
				showLODOptions = true;
			}
			...
			switch (currentStructureView) {
				case STRUCTURE_BRANCH: // BRANCHES.
					...
					if (branchGirthFoldout) {
						...
						girthAtBase = EditorGUILayout.Slider (labelGirthAtBase, selectedBranchDescriptor.girthAtBase, 0.005f, 0.1f);
						if (girthAtBase != selectedBranchDescriptor.girthAtBase) {
							changed = true;
						}
						girthAtTop = EditorGUILayout.Slider (labelGirthAtTop, selectedBranchDescriptor.girthAtTop, 0.001f, 0.02f);
				...
				case STRUCTURE_SPROUT_A: // LEAVES.
					...
					EditorGUI.BeginChangeCheck ();
					sproutASize = EditorGUILayout.Slider (labelSize, selectedBranchDescriptor.sproutASize, 0.005f, 0.3f);
					...
					sproutANormalRandomness = EditorGUILayout.Slider (labelAlignmentRandom, selectedBranchDescriptor.sproutANormalRandomness, 0f, 1f);
					changed |= EditorGUI.EndChangeCheck ();
					EditorGUILayout.Space ();
				...
				case STRUCTURE_SPROUT_B: // LEAVES.
					...
					sproutBNormalRandomness = EditorGUILayout.Slider (labelAlignmentRandom, selectedBranchDescriptor.sproutBNormalRandomness, 0f, 1f);
					changed |= EditorGUI.EndChangeCheck ();
					EditorGUILayout.Space ();
					// Draw Sprout A Hierarchy Structure Panel
			...
			if (changed) {
				...
				selectedBranchDescriptor.sproutANormalRandomness = sproutANormalRandomness;
				...
				selectedBranchDescriptor.sproutBNormalRandomness = sproutBNormalRandomness;
				onEditBranchDescriptor?.Invoke (selectedBranchDescriptor, branchDescriptorCollection);
				ReflectChangesToPipeline ();
				RegeneratePreview ();
			}
				...
				if (branchFoldouts [i]) {
					...
					if (i == 0) {
						...
						changed |= BroccoEditorGUI.FloatRangePropertyField (
							...
							0.1f, 2f, currentStructureSettings.branchEntitiesName + " Length");
						// RADIUS
						if (currentStructureSettings.displayRadiusControl) {
							float newRadius = EditorGUILayout.Slider (labelRadius, proxyBranchLevelDescriptor.radius, 0f, 6f);
		...
		public void DrawDebugPanel () {
			...
			switch (currentDebugView) {
				case DEBUG_GEOMETRY: // GEOMETRY.
					...
					EditorGUILayout.EndHorizontal ();
					if (selectedBranchDescriptor != null) {
						string snapshotInfo = $"Snapshot [id: {selectedBranchDescriptor.id}], Curve [{selectedBranchDescriptor.curve.nodeCount} nodes]";
				...
				case DEBUG_BUILDER: // BUILDERS.
					...
					GUILayout.FlexibleSpace ();
					EditorGUILayout.EndHorizontal ();
					if (MeshEditorDebug.Current ().DrawEditorOptions (meshPreview))
		...
		private void DebugRescaleDescriptor (float scale) {
			...
			for (int i = 0; i < branchDescriptorCollection.branchDescriptors.Count; i++) {
				snapshot = branchDescriptorCollection.branchDescriptors [i];
				// Scale girths.
		...
		void RemoveSnapshot () {
			if (EditorUtility.DisplayDialog (MSG_DELETE_BRANCH_DESC_TITLE, 
				...
				branchDescriptorCollection.lastBranchDescriptorIndex = branchDescriptorCollection.branchDescriptorIndex;
				onBeforeEditBranchDescriptor?.Invoke (selectedBranchDescriptor, branchDescriptorCollection);
				branchDescriptorCollection.RemoveBranchDescriptor (selectedBranchDescriptor.id);
				onEditBranchDescriptor?.Invoke (selectedBranchDescriptor, branchDescriptorCollection);
				SelectSnapshot (0);
				ReflectChangesToPipeline ();
				RegeneratePreview ();
		...
		void AddVariation () {
			if (branchDescriptorCollection.variationDescriptors.Count < 15) {
				...
				onBeforeEditVariationDescriptor?.Invoke (selectedVariationDescriptor, branchDescriptorCollection);
				branchDescriptorCollection.AddVariationDescriptor (newVariationDescriptor);
				onEditVariationDescriptor?.Invoke (selectedVariationDescriptor, branchDescriptorCollection);
				InitVariationViewOptions ();
		...
		void RemoveVariation () {
			if (EditorUtility.DisplayDialog (MSG_DELETE_VARIATION_DESC_TITLE, 
				...
				branchDescriptorCollection.lastVariationDescriptorIndex = branchDescriptorCollection.variationDescriptorIndex;
				onBeforeEditVariationDescriptor?.Invoke (selectedVariationDescriptor, branchDescriptorCollection);
				branchDescriptorCollection.RemoveVariationDescriptor (selectedVariationDescriptor);
				SelectVariation (0);
				onEditVariationDescriptor?.Invoke (selectedVariationDescriptor, branchDescriptorCollection);
				GUIUtility.ExitGUI ();
			}
		...
		public void OnPreviewMeshDrawHandles (Rect r, Camera camera) {
			...
			if (currentImplementation != null) {
				currentImplementation.OnCanvasDrawHandles (r, camera);
			}
			// SNAPSHOT.
			if (canvasStructureView == CanvasStructureView.Snapshot && selectedBranchDescriptor != null) {
				...
				if (GlobalSettings.sproutLabDebugEnabled && Event.current.type == EventType.Repaint) {
					//DebugDrawVectors.current.DrawGroupA ();
					//DebugDrawVectors.current.DrawGroupB ();
					...
					for (int pI = 0; pI < polygonAreas.Count; pI++) {
						if (debugPolyIndex == 0 || pI == (debugPolyIndex - 1)) {
							...
							#if BROCCOLI_DEVEL
							if (debugShowCurve) {
								BezierCurveDraw.DrawCurve (selectedBranchDescriptor.curve, Vector3.zero, sproutSubfactory.factoryScale, Color.yellow, 5f);
								BezierCurveDraw.DrawCurveNodes (selectedBranchDescriptor.curve, Vector3.zero, sproutSubfactory.factoryScale, Color.yellow);	
							}
		...
		public void DrawLODViewControls (Rect r) {
			bool drawLOD = true;
			if (currentImplementation != null) {
				drawLOD = currentImplementation.DrawLODControls (r);
			}
			...
			out string subsurfacePath)
		{
			...
			albedoPath = GetTextureFileName (subfolder, branchDescriptorCollection.exportTake, out isValid, SproutSubfactory.MaterialMode.Albedo, true);
			normalPath = GetTextureFileName (subfolder, branchDescriptorCollection.exportTake, out isValid, SproutSubfactory.MaterialMode.Normals, true);
			extrasPath = GetTextureFileName (subfolder, branchDescriptorCollection.exportTake, out isValid, SproutSubfactory.MaterialMode.Extras, true);
			subsurfacePath = GetTextureFileName (subfolder, branchDescriptorCollection.exportTake, out isValid, SproutSubfactory.MaterialMode.Subsurface, true);
		...
		public string GetTextureFileName (string subfolder, int take, out bool isValid, SproutSubfactory.MaterialMode materialMode, bool isAtlas) {
			isValid = true;
			string path = "";
